 ...
 * @author chanc
 */
@WebServlet(name = "DistributionReportServlet", urlPatterns = {"/DistributionReportServlet"})
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBConnection.getConnection()) {
            StringBuilder query = new StringBuilder(
                    "SELECT COALESCE(c.name, d.customer_name) AS customer_name,COALESCE(c.contact_number, d.contact_number) AS contact_number, d.distribution_date,d.shift,d.quantity_liters,d.rate_per_liter,d.total_amount FROM distribution d LEFT JOIN customer c ON d.customer_id = c.customer_id WHERE d.distribution_date BETWEEN ? AND ?"
            );
